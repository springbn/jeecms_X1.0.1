...
public class ContentLuceneServiceImpl implements ContentLuceneService, ContentListener {
	...
	public boolean resetIndex(Integer channelId, Integer siteId, Date releaseTimeStart, Date releaseTimeEnd)
		...
		ThreadPoolService.getInstance().execute(new Runnable() {
			...
			public void run() {
				for (int i = 0; i < totalPage; i++) {
					...
					try {
						content = contentGetService.getPages(query, pageable);
						createIndexBatch(content.getContent());
		...
		logger.info("resetIndex query times " + (t2 - t1));
		logger.info("resetIndex create lucene  times " + (t3 - t2));
		logger.info("resetIndex content count " + content.getTotalElements());
		return true;
	}
