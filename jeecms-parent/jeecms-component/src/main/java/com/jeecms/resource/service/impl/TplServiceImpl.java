...
public class TplServiceImpl implements TplService {
        ...
        for (String name : names) {
            ...
            if (f.isDirectory()) {
                ...
                if (f.delete()) {
                    ...
                    int num = fname.indexOf(WebConstants.SPT);
                    if (num != -1) {
                        fname = fname.substring(num + 1);
                    }
                    List<CmsModelTpl> list = modelTplService.findByTplPath(site.getId(), fname);
    ...
    public void rename(String orig, String dist, CmsSite site) throws GlobalException {
        ...
        try {
            if (of.isDirectory()) {
                ...
                //查询模板关联模型数据，修改模板路径
                List<CmsModelTpl> modelTpls = modelTplService.findByTplPath(null, dist);
                for (CmsModelTpl modelTpl : modelTpls) {
    ...
    public void update(TplUpdateDto dto, CmsSite site) throws GlobalException {
        ...
        try {
            ...
            List<CmsModelTpl> list = modelTplService.findByTplPath(site.getSiteId(), fname);
            modelTplService.physicalDeleteInBatch(list);
            //添加模板模型关联
