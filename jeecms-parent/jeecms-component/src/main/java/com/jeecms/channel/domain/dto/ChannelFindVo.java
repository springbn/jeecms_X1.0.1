...
public class ChannelFindVo {
	...
	public ChannelFindVo splicChannelFindVo(Channel channel, GlobalConfig globalConfig, CmsModel model,
		...
		// 默认字段
		Set<CmsModelItem> defaultModelItems = modelItems.stream().filter(modelItem -> !modelItem.getIsCustom())
	 ...
	 * 初始化默认字段
	 */
	private JSONObject initDefaultModelItems(Set<CmsModelItem> items, Channel channel, Map<String, String> txtMap) {
	 ...
	 * 初始化自定义字段
	 */
	private JSONObject initCustomModelItems(Set<CmsModelItem> items, List<ChannelAttr> channelAttrs,
		...
		if (channelAttrs != null && channelAttrs.size() > 0) {
			attrMap = channelAttrs.stream()
					.collect(Collectors.toMap(ChannelAttr::getAttrName, ChannelAttr -> ChannelAttr));
		}
