...
public class ContentDocServiceImpl extends AbstractContentListener {
	...
	public void upload(Integer contentId, CmsSite site) throws GlobalException {
		...
		String fileUrl = "";
		ResourcesSpaceData space = content.getContentExt().getDocResource();
		// 得到内容原始文档路径
		if (space != null) {
			// 得到默认上传路径
			...
			// 得到doc绝对路径
			String docRealPath = realPathResolver.get(origName);
			//pdf输入路径
			...
			try {
				...
				// 默认pdf
				String ext = "pdf";
				if (UploadServerType.ftp.equals(site.getUploadServerType()) 
						&& site.getUploadFtp() != null) {
		...
		content.getContentExt().setPdfUrl(fileUrl);
		//修改pdf文件路径
		contentService.update(content);
