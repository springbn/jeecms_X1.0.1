...
public class ContentFrontServiceImpl implements ContentFrontService {
	...
	public void contribute(ContentContributeDto dto) throws GlobalException {
		...
		if (items != null && items.size() > 0) {
			...
			if (fields.contains(CmsModelConstant.CONTENT_TXT)) {
				Map<String, String> txtMap = new HashMap<String, String>();
				txtMap.put(CmsModelConstant.CONTENT_TXT, dto.getContxt());
				...
				ContentTxt contentTxt = new ContentTxt(CmsModelConstant.CONTENT_TXT, dto.getContxt(), bean.getId());
			...
			if (fields.contains(CmsModelConstant.CONTENT_TXT)) {
				...
				if (contentTxts != null && contentTxts.size() > 0) {
					for (ContentTxt contentTxt : contentTxts) {
						if (CmsModelConstant.CONTENT_TXT.equals(contentTxt.getAttrKey())) {
	...
	@Override
	public ContentContributeVo splicContributeVo(Content content) {
	...
	@Override
	public Page<ContentMobileVo> getMobilePage(Integer siteId, Integer userId, Integer status, Pageable pageable) throws GlobalException{
		/**
		...
		List<ContentMobileVo> vos = this.initMobileVos(contents);
		vos = vos.stream()
				.skip(pageable.getPageSize() * (pageable.getPageNumber()))
				.limit(pageable.getPageSize()).collect(Collectors.toList());
		Page<ContentMobileVo> page = new PageImpl<ContentMobileVo>(vos, pageable, vos.size());
		return page;
	}
	 ...
	 * @return: List
	 */
	private List<ContentMobileVo> initMobileVos(Page<Content> contents)  throws GlobalException{
		List<ContentMobileVo> vos = new ArrayList<ContentMobileVo>();
		for (Content content : contents) {
			ContentMobileVo vo = new ContentMobileVo();
	 ...
	 */
	public ContentMobileVo initMobileVo(ContentMobileVo vo, Content content) throws GlobalException {
		Integer contentId = content.getId();
		vo.setId(contentId);
		vo.setTitle(content.getTitle());
	 ...
	 * @return: ContentMobileVo
	 */
	private ContentMobileVo initMobileNumVo(ContentMobileVo vo, JSONObject json) {
