...
//		flowService.doInterruptFlow(flowIds);
	...
	public ResponseInfo changeStatus(BeatchDto dto,CoreUser user) throws GlobalException {
		...
		// 判断前台传的状态包含的发布，下线，归档，是否有操作权限，因为发布与下线、归档当作权限一体；
		Boolean flag = dto.getStatus().equals(STATUS_PUBLISH) || dto.getStatus().equals(STATUS_NOSHOWING)
				|| dto.getStatus().equals(STATUS_PIGEONHOLE);
		...
		if (dto.isCheckPerm()) {
			...
			Map<Integer, List<Content>> map = contents.stream()
		...
		for (Content content : contents) {
			...
			list.add(content);
			records.add(new ContentRecord(content.getId(), user.getUsername(),
					"修改状态为" + ContentConstant.status(dto.getStatus())));
            ...
            if (bean.getStatus().equals(STATUS_PUBLISH)) {
                ...
                hotWordService.totalUserCount(bean.getChannelId(), contentTxts);
            }
			...
			if (workflow != null) {
				...
				switch (content.getStatus()) {
					...
					case STATUS_PUBLISH:
						...
						if (!flag) {
							...
							if (!Arrays.asList(STATUS_DRAFT, STATUS_FIRST_DRAFT, STATUS_PUBLISH,
									STATUS_NOSHOWING, STATUS_FLOWABLE,
									STATUS_PIGEONHOLE).contains(status)) {
								throw new GlobalException(ContentErrorCodeEnum.CONTENT_STATUS_ERROR);
							}
						}
					...
					case STATUS_NOSHOWING:
						...
						if (!Arrays
								.asList(STATUS_DRAFT, STATUS_FIRST_DRAFT, STATUS_FLOWABLE,
										STATUS_PUBLISH, STATUS_PUBLISH, STATUS_PIGEONHOLE)
								.contains(status)) {
							throw new GlobalException(ContentErrorCodeEnum.CONTENT_STATUS_ERROR);
						}
						break;
				...
				switch (content.getStatus()) {
					...
					case STATUS_PUBLISH:
						...
						// 判断站点设置是否允许内容发布编辑
						if (!flag) {
							if (!Arrays.asList(STATUS_NOSHOWING,
									STATUS_PIGEONHOLE).contains(status)) {
								throw new GlobalException(ContentErrorCodeEnum.CONTENT_STATUS_ERROR);
							}
						} else {
	...
	public ResponseInfo rubbish(OperationDto dto) throws GlobalException {
		...
		super.batchUpdate(contents);
		if (contents.size() > 0) {
			List<Integer> contentIds = contents.stream().filter(content -> content.getStatus().equals(STATUS_FLOWABLE)).map(Content::getId).collect(Collectors.toList());
	...
	public ResponseInfo quote(OperationDto dto) throws GlobalException {
		...
		for (Integer channelId : list) {
			List<Short> oprations = SystemContextUtils.getUser(RequestUtils.getHttpServletRequest())
					.getContentOperatorByChannelId(channelId);
		...
		// 检测是否引用内容原有的栏目
		for (Content content : contents) {
			if (channels.contains(content.getChannelId())) {
	...
	@Override
	public Content save(ContentSaveDto dto, HttpServletRequest request) throws GlobalException {
		...
		content = dto.initContent(dto, content, site, globalConfig, false, channel);
		// 校验传送过来的状态
		...
		ContentChannel contentChannel = new ContentChannel(bean.getId(), bean.getChannelId(),
				ContentConstant.CONTENT_CREATE_TYPE_ADD, bean.getStatus(), false, bean, false);
		...
		bean = initAttr(bean, dto.getJson(), dto.getModelId(), bean.getId(), globalConfig);
	...
	public ContentFindVo findContent(Integer id, GlobalConfig globalConfig) throws GlobalException {
		Content content = super.findById(id);
		...
		findVo = findVo.spliceContentFindVo(content, globalConfig, model, modelItems,
				content.getContentExt().getContentSource(), cmsOrgMap);
		/** 组装contentFindVo对象 */
	 ...
	 * @throws GlobalException
	 */
	private Content initAttr(Content content, JSONObject json, Integer modelId, Integer id,
	...
	public Content update(ContentUpdateDto dto, HttpServletRequest request) throws GlobalException {
		...
		// 校验传送过来的状态
		ContentInitUtils.checkStatus(content.getStatus(), dto.getType(),
				channel.getRealWorkflowId() != null, true);
	...
	@Override
	public void submit(ContentUpdateDto dto, HttpServletRequest request) throws GlobalException {
	...
	public void copy(ContentCopyDto dto, HttpServletRequest request) throws GlobalException {
		...
		for (Integer i = 0; i < dto.getIds().size(); i++) {
			...
			List<ContentAttr> contentAttrs = contentAttrService.copyInitContentAttr(content.getContentAttrs(),
					bean.getId());
			List<ContentAttr> newContentAttrs = contentAttrService.saveAll(contentAttrs);
	...
	public boolean checkTitle(String title, Integer channelId) {
		...
		Long contentIdConunt = contentLuceneService.searchCount(title, SearchPosition.title,
		                channelId, siteId, true, ContentConstant.getNotAllowRepeatStatus());
	...
	public String replaceImg(String html, String path) throws IOException, GlobalException {
		...
		for (org.jsoup.nodes.Element element : imgTags) {
			...
			// 上传文件
			UploadResult result = uploadService.doUpload(file, null, null, 
					ResourceType.IMAGE, site);
			//应前端需求图片地址给绝对地址
	...
	public void restore(List<Integer> contentIds, Integer siteId, List<Integer> channelIds) throws GlobalException {
		...
		for (Content content : list) {
			...
			if (contentChannels.size() > 1) {
				...
				contentChannels.get(0).setRecycle(false);
				contentChannelList.add(contentChannels.get(0));
			}
