...
//										ContentErrorCodeEnum
//										.ATTACHMENTS_MUST_BE_ENCRYPTED
//										.getDefaultMessage(),
//										ContentErrorCodeEnum
//										.ATTACHMENTS_MUST_BE_ENCRYPTED
//										.getCode()));
...
//						}
	...
	@Override
	public List<ContentAttr> copyInitContentAttr(List<ContentAttr> contentAttrs, Integer contentId) {
		/**
		 * 底下无需如上方一样判断因为这是copy，copy如果出现上面的未判断导致出错的问题说明我save、update处理有问题，此处修改了也会有问题
		 */
		for (ContentAttr contentAttr : contentAttrs) {
			ContentAttr newContentAttr = new ContentAttr();
			MyBeanUtils.copyProperties(contentAttr, newContentAttr);
			newContentAttr.setId(null);
			contentAttr.setContentId(contentId);
			List<ContentAttrRes> contentAttrRes = contentAttr.getContentAttrRes();
			if (contentAttrRes != null && contentAttrRes.size() > 0) {
				for (ContentAttrRes attrRes : contentAttrRes) {
					ContentAttrRes newAttrRes = new ContentAttrRes();
					MyBeanUtils.copyProperties(attrRes, newAttrRes);
					newAttrRes.setContentAttrId(null);
					newAttrRes.setContentAttr(contentAttr);
				}
			}
		}
		return contentAttrs;
...
//							}
							...
							attrRes.setSecretId(null);
						}
					}
				}
				newContentAttrs.add(contentAttr);
			}
		}
