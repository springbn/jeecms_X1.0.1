...
public class LogAlarmJob implements IBaseJob {
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		initService();
		if (logConfigService == null || messageService == null) {
			return;
		}
	...
	private void sendEmail(String emailList, Integer dangerValue, Float size) {
		...
		if (dangerValue <= size) {
			...
			if (emails != null) {
				try {
					messageService.sendMemberMsg(null, null, null, null,
							null, null, MessageSceneEnum.USER_MESSAGE,
							MessageTempleEnum.LOG_DANGER_TEMP.getTitle(),
							content(dangerValue), null, Arrays.asList(emails), new JSONObject(),
							MqConstants.SEND_EMAIL, null);
	...
	private void sendSMS(String smsList, Integer dangerValue, Float size, String dangerSmsTmpId) {
		...
		if (dangerValue <= size) {
			...
			if (phones != null) {
				try {
					messageService.sendMemberMsg(null, null, null, null, null,
							dangerSmsTmpId, MessageSceneEnum.USER_MESSAGE, MessageTempleEnum.LOG_DANGER_TEMP.getTitle(),
