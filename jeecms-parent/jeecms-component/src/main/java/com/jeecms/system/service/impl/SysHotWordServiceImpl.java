...
public class SysHotWordServiceImpl extends BaseServiceImpl<SysHotWord, SysHotWordDao, Integer>
	...
	public List<SysHotWord> getList(String hotWord, Integer siteId, Integer hotWordCategoryId, Integer channelId,
									Paginable paginable) {
		return dao.getList(hotWord, siteId, hotWordCategoryId, channelId, paginable);
	}
	...
	public String attachKeyword(Integer channelId, String txt, CmsSite site) {
		...
		List<SysHotWord> list = getList(null, null, null, channelId, new PaginableRequest());
		int len = list.size();
		if (len <= 0) {
			return txt;
		}
	...
	@Transactional(readOnly = true)
	@Override
	public void totalUserCount(Integer channelId, List<ContentTxt> contentTxts) throws GlobalException {
		List<SysHotWord> list = getList(null, null, null, channelId, new PaginableRequest());
		int len = list.size();
		if (len <= 0) {
			return;
