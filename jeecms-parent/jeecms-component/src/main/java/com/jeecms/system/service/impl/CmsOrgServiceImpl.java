...
public class CmsOrgServiceImpl extends BaseServiceImpl<CmsOrg, CmsOrgDao, Integer>
        ...
        @Override
        public ResponseInfo sort(Integer orgId, Integer orderOrgId, Boolean up) throws GlobalException {
                CmsOrg org = super.findById(orgId);
                CmsOrg orderOrg = super.findById(orderOrgId);
                if (up) {
                        org.setSortWeight(org.getSortWeight() + 1);
                        orderOrg.setSortWeight(orderOrg.getSortWeight() - 1);
                } else {
                        org.setSortWeight(org.getSortWeight() - 1);
                        orderOrg.setSortWeight(orderOrg.getSortWeight() + 1);
                }
                super.update(org);
                super.update(orderOrg);
                return new ResponseInfo();
        }
