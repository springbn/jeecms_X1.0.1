	...
	private void cmsSiteConfig(Sms sms) throws GlobalException {
		...
		if (sms.getIsGloable() != null && sms.getIsGloable()) {
			for (CmsSite cmsSite : cmsSiteList) {
				...
				cmsSiteConfig.setServiceProviders(String.valueOf(sms.getServiceProvider()));
				cmsSiteConfig.setAccesskeyId(sms.getAccessKey());
				cmsSiteConfig.setAccesskeySecret(sms.getAccesskeySecret());
				cmsSiteConfig.setMessageSignatures(sms.getSmsSign());
			}
		} else {
			for (CmsSite cmsSite : cmsSiteList) {
				CmsSiteConfig cmsSiteConfig = cmsSite.getConfig();
				cmsSiteConfig.setServiceProviders("");
				cmsSiteConfig.setAccesskeyId("");
				cmsSiteConfig.setAccesskeySecret("");
				cmsSiteConfig.setMessageSignatures("");
			}
		}
